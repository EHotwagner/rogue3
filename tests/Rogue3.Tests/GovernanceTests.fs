module Rogue3GovernanceTests

open System
open Expecto

// Feature 060 (FR-005): durable, model-agnostic governance scans. These read the
// generated rogue3 SOURCE TEXT (and build.fsx) and assert structural / evidence /
...
// passing because it never calls the rogue3's `view`/`update`.
...
let rogue3Source file =
    System.IO.File.ReadAllText(System.IO.Path.Combine(__SOURCE_DIRECTORY__, "..", "..", "src", "Rogue3", file))

let rogue3Sources files =
    files |> List.map rogue3Source |> String.concat "\n"
...
// Feature 242 (spec 242-scaffold-discoverability, roadmap #75): a generated rogue3 ships a
...
// (tests/Package.Tests/SwapChecklistTemplateTests.fs), not a rogue3 scan.
let rogue3RootFile file =
    System.IO.File.ReadAllText(System.IO.Path.Combine(__SOURCE_DIRECTORY__, "..", "..", file))

let assertSwapChecklistPresent () =
    let checklist = rogue3RootFile "SWAP-CHECKLIST.md"
...
// script trigger is the bare `help` token (T004 live check). The banner↔docs/rogue3.md SYNC
// (FR-009/SC-004) is a template-authoring gate (tests/Package.Tests/SwapChecklistTemplateTests.fs)
// rather than a rogue3 scan — that gate reads the template docs verbatim, whereas here a generated
// tree rewrites the `rogue3` token, so it cannot honestly read docs/rogue3.md by name.
...
// `generatedHost`. This is a host value the rogue3 names, not a launcher overload — a sanctioned
...
[<Tests>]
let governanceTests =
    testList "rogue3-governance" [
        test "generated rogue3 source is split by responsibility in compile order" {
            let rogue3Dir = System.IO.Path.Combine(__SOURCE_DIRECTORY__, "..", "..", "src", "Rogue3")
            let project = System.IO.File.ReadAllText(System.IO.Path.Combine(rogue3Dir, "Rogue3.fsproj"))

            [ "Model.fs"; "View.fs"; "LayoutEvidence.fs"; "WindowOptions.fs"; "EvidenceCommands.fs"; "Program.fs" ]
            |> List.iter (fun file ->
                Expect.isTrue (System.IO.File.Exists(System.IO.Path.Combine(rogue3Dir, file))) $"{file} exists in generated rogue3 source"
            ...
            let program = System.IO.File.ReadAllText(System.IO.Path.Combine(rogue3Dir, "Program.fs"))
        ...
        test "generated graphical app exposes bounded smoke command" {
            let source = rogue3Sources [ "Program.fs"; "EvidenceCommands.fs" ]

            Expect.stringContains source "--launch-evidence" "generated rogue3 exposes explicit launch evidence CLI"
            ...
            Expect.stringContains source "--bounded-smoke" "generated rogue3 exposes bounded smoke CLI"
            Expect.stringContains source "--bounded-smoke-frame-diagnostics" "generated rogue3 exposes explicit frame diagnostic smoke CLI"
        ...
        test "generated evidence commands are opt-in and not reported as ongoing interactive play" {
            let source = rogue3Sources [ "Program.fs"; "EvidenceCommands.fs" ]
            let program = rogue3Source "Program.fs"
        ...
        test "generated visual evidence commands require screenshot proof pixel fallback and unsupported diagnostics" {
            ...
            Expect.stringContains source "--image-evidence" "generated rogue3 exposes image evidence command"
            Expect.stringContains source "--screenshot-evidence" "generated rogue3 exposes screenshot evidence command"
            Expect.stringContains source "--pixel-readback-evidence" "generated rogue3 exposes pixel-readback evidence command"
        ...
        test "generated evidence commands share Testing report conventions" {
            ...
            Expect.stringContains source "let writeEvidenceReport" "generated rogue3 defines one local report wrapper"
            Expect.stringContains source "generatedEvidenceStatusText" "generated rogue3 shares normalized report status vocabulary"
            Expect.stringContains source "| GeneratedEvidenceOk -> \"ok\"" "generated rogue3 preserves ok status vocabulary"
            Expect.stringContains source "| GeneratedEvidenceUnsupported -> \"unsupported\"" "generated rogue3 preserves unsupported status vocabulary"
            Expect.stringContains source "| GeneratedEvidenceFailed -> \"failed\"" "generated rogue3 preserves failed status vocabulary"
            Expect.stringContains source "generatedEvidenceExitCode" "generated rogue3 keeps report status to exit-code semantics local"
        ...
        test "generated graphical app default executable path uses persistent host" {
            ...
            Expect.stringContains source "let viewerOptions" "generated rogue3 declares viewer options"
            Expect.stringContains source "let generatedHost" "generated rogue3 declares generated host"
        ...
        test "generated normal launch reports desktop session diagnostics without evidence fallback" {
            let source = rogue3Source "Program.fs"
        ...
        test "generated window diagnostics command derives its verdict from the real launch gate, not fabricated failures" {
            let source = rogue3Sources [ "Program.fs"; "EvidenceCommands.fs" ]
            let evidence = rogue3Source "EvidenceCommands.fs"
            let program = rogue3Source "Program.fs"
            let defaultBranch = program.Substring(program.LastIndexOf("| None ->", StringComparison.Ordinal))

            Expect.stringContains source "--window-diagnostics" "generated rogue3 exposes an explicit window diagnostics command"
            ...
            Expect.stringContains source "diagnostic-class=rogue3-defect" "diagnostics include rogue3 defect class"
            ...
            Expect.isFalse (evidence.Contains "rogue3 requested a zero-sized or surface-less window") "window diagnostics no longer fabricate a rogue3-defect failure (#136)"
        ...
        test "generated app Synthetic exposes window behavior flags and option diagnostics without leaving interactive launch" {
            let source = rogue3Sources [ "Program.fs"; "WindowOptions.fs" ]
            let program = rogue3Source "Program.fs"
            let defaultBranch = program.Substring(program.LastIndexOf("| None ->", StringComparison.Ordinal))
            ...
            Expect.stringContains source "--window-options" "generated rogue3 exposes option diagnostics"
        ...
        test "generated graphical app exposes deterministic scene evidence command" {
            ...
            Expect.stringContains source "--scene-evidence" "generated rogue3 exposes non-window scene evidence CLI"
        ...
        test "generated rogue3 ships the SWAP-CHECKLIST discoverability doc" {
