...
- AC-001 [US-003] [FR-001]: Given the product's cue id declaration, when the guard test enumerates the ids it must cover, then the enumeration is read from that declaration - the same values `AudioCues.fs` builds its `SoundId`/`TrackId` values from - and no cue id string is written into the test.
...
- AC-005 [US-003] [FR-005]: Given `src/Rogue3/AudioCues.fs` with its comments stripped, when it is scanned for `SoundId "..."`, `TrackId "..."` and `sfx "..."` literals, then every literal found is a value the declaration exports, so a cue added with an inline literal cannot escape the declared set.
- AC-006 [US-002] [FR-006]: Given the parameterized floor-theme family `floor-<n>-theme`, when it is evaluated at every integer a run can reach and beyond it, then every result is a member of the declared track set, so no floor index can request an undeclared track.
- AC-007 [US-004] [FR-007]: Given the committed generator script and an empty `assets/audio/`, when the script is run, then it writes exactly one `.wav` per declared id and no others, and re-running it reproduces every file byte for byte.
...
- FR-005: No cue id literal MAY appear in `AudioCues.fs` outside the declaration; a scan of the file with comments stripped MUST find only literals the declaration exports. (covers AC-005)
- FR-006: The parameterized `floor-<n>-theme` family MUST be total over every integer floor index - clamped into the declared range - so no reachable or unreachable floor index can request a track that is not declared. (covers AC-006)
