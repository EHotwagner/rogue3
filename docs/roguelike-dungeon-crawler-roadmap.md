...
- 🟥 A room transition renders both the room being left and the room being entered, so a crossing reads as a slide rather than a blank screen — `Render.cameraOffset` translates the nine world layers a full playfield width or height at `remaining = 1.0` (M6's "entry begins one room away"), and nothing draws the departed room, so every door a player crossed would have shown an empty screen for the full 0.35 s (`m6CameraDurationTicks = 42`); M11 stopped starting the slide rather than ship that
...
Reason for this milestone (2026-08-02): every row here was found by looking at M11's production frames — four of them by an independent visual-coverage critic — and none of them is reachable by a requirements-derived test. The transition row is the sharpest case, and it is a second-order consequence of M11 rather than old debt: nothing dispatched `TraverseDoor` before M11, so the M6 camera slide was reachable only from an evidence-only message and had never actually been seen. Making traversal reachable made the slide reachable, and the first frames rendered after a crossing came back blank apart from the HUD. The remaining rows: `M5ObstacleDrops` carries no position at all; `RoomReward`/`ShopItem` sit at fixed screen coordinates and overlap obstacles; the player's collision bound is the playfield rather than the drawn wall, so the player renders inside it; and `HudScore` is one catalogue row covering hearts, currency, charge, banner and the whole minimap, so deleting the boss-room minimap colour would keep the coverage audit complete.

### M14 — Scripted play-through agent & feature reachability audit
- 🟥 A deterministic scripted agent drives the game **only** through the production input route — the same seam a human uses. It may not construct model state, and it may not dispatch a `Msg` the input route cannot produce
- 🟥 The agent completes a full seeded run: boot → floor 1 → descend to floor 6 → defeat the final boss → `Victory`. The milestone fails closed if the agent cannot win
- 🟥 The agent solves the real obstacles rather than being handed them — a `LockedKey` door requires actually holding a key, a `BossDoor` requires the room cleared, a secret room requires a bomb detonated adjacent to it, a trapdoor requires the boss beaten and the player standing on it
- 🟥 The run is deterministic and replayable: seed plus the agent's captured input log reproduces a byte-identical final model through the M10 canonical encoder (`src/Rogue3/Determinism.fs`), and a different seed or different tick timing does not
- 🟥 Feature reachability audit: every roadmap AC and feature row from M0–M13 is classified `exercised-through-play`, `exercised-only-by-direct-dispatch`, or `unreachable-by-play`, published as a committed artifact
- 🟥 Every `unreachable-by-play` classification is either fixed in this milestone or recorded on the roadmap as explicit, named deferred work — none is left silent
- 🟥 The audit runs in CI and fails closed when a feature's only evidence is a direct `update` call or a constructed `{ initialModel with … }` state
- 🟥 The agent's run emits render-and-look evidence per floor — committed PNGs through `Render.toPng`, visually inspected, with the final-boss and `Victory` frames among them

Reason for this milestone (2026-08-02): the suite proves mechanisms, not paths. Measured at `3dbf7b3` with `grep -rohE '^[[:space:]]*test(Case)? "' tests/Rogue3.Tests/*.fs`, the suite held 158 test declarations against 153 direct `update <Msg>` calls and 48 hand-constructed `{ initialModel with … }` states, with only 51 drives through a journey or host route; at `97a352a` the same commands give 173, 167 and 48 against 64. The M9 victory test reaches the final boss by assignment — `FloorIndex=6`, the boss spawned at the player's own position and forced to `HitPoints=1.0`, the projectile spawned already overlapping it, and `RunStats` backdated to `DepthReached=6; BossKills=2; FloorsCleared=5` — and `DescendFloor` descended with no trapdoor, cleared-state or position precondition until M11 guarded it. Each of those claims is individually true and the assertions run through the real reducer; none of them required a player to move. M11 closed the sealed starting room, and M12 and M13 close the silent cue set and the blank room crossing, but each was found by a human looking rather than by a gate. An agent that must actually win, through the only seam a human has, cannot be satisfied by a record literal — which is the property this milestone exists to buy. It is sequenced after M13 because a run it must win still crosses doors that cannot yet render.
