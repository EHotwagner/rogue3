#!/usr/bin/env bash
# Feature 212: uniform rogue3-root verb wrapper. Every verb delegates to the generated FSI script
# (`dotnet fsi build.fsx -t <Target>`), which is the rich/governed path. Stock
# `dotnet build/test/run` at the rogue3 root remain independently usable via <Name>.slnx.
...
# green Dev is never mistaken for a passing compile. Kept in sync with docs/rogue3.md + build.fsx.
